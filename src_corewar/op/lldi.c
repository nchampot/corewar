...
/*   Updated: 2016/11/17 13:32:23 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		op_ldi(t_env *env, int num_player, unsigned int op_args[4], char codage)
{
	...
	if (is_register(op_args[0], codage, 0))
		...
		value = op_args[0] + env->champion[num_player]->program_control;
	...
	if (!is_register(op_args[2], codage, 2))
		return (-1);
	ft_strncpy(env->champion[num_player]->reg[op_args[2]], &env->memory[env->champion[num_player]->program_control + value], REG_SIZE * 2);
