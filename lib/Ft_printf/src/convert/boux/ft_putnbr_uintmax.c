...
/*   Updated: 2016/04/04 15:52:58 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/convert_boux.h"

void		ft_p_uintmax(uintmax_t n, int base, int *nb_written, t_print param)
{
	char c;
	...
	if (n >= (uintmax_t)base)
	{
		ft_p_uintmax(n / base, base, nb_written, param);
		ft_p_uintmax(n % base, base, nb_written, param);
	}
	else
	{
...
void		ft_calc_uintmax(int nb_char[2], uintmax_t n, t_print param, \
	int base)
{
	int	size;

	size = 0;
	ft_len_uintmax(n, base, &size);
...
void		suite_uintmax_2(uintmax_t n, t_print param, int *nb_written)
{
...
void		suite_uintmax(uintmax_t n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
	...
	suite_uintmax_2(n, param, nb_written);
...
int			ft_putnbr_uintmax(uintmax_t n, t_print param, int base)
{
	int	i;
	int	nb_char[2];
	int	nb_written;
	...
	i = 0;
	nb_written = 0;
	ft_calc_uintmax(nb_char, n, param, base);
	suite_uintmax(n, param, nb_char, &nb_written);
	...
	if (!(n == 0 && param.precision == 0 && param.precision_spec))
		ft_p_uintmax(n, base, &nb_written, param);
	i = 0;
