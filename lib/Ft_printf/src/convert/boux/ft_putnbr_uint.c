...
/*   Updated: 2016/04/04 16:54:08 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/convert_boux.h"

void			ft_p_uint(unsigned int n, int base, int *nb_written, \
	t_print param)
{
	char c;
	...
	{
		ft_p_uint(n / base, base, nb_written, param);
		ft_p_uint(n % base, base, nb_written, param);
	}
	else
	{
...
void			ft_calc_uint(int nb_char[2], unsigned int n, t_print param, \
	int base)
{
	int	size;

	size = 0;
	ft_len_uint(n, base, &size);
...
void			suite_uint_2(unsigned int n, t_print param, int *nb_written)
{
...
void			suite_uint(unsigned int n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
	...
	suite_uint_2(n, param, nb_written);
	while (i < nb_char[1] && param.flags[1] == 1 && param.flags[2] == 0 \
			&& param.precision == 0 && n != 0)
	{
		ft_putchar('0');
		(*nb_written)++;
		i++;
...
int				ft_putnbr_uint(unsigned int n, t_print param, int base)
{
	int	i;
	int	nb_char[2];
	int	nb_written;
	...
	i = 0;
	nb_written = 0;
	ft_calc_uint(nb_char, n, param, base);
	suite_uint(n, param, nb_char, &nb_written);
	...
	if (!(n == 0 && param.precision == 0 && param.precision_spec))
		ft_p_uint(n, base, &nb_written, param);
	i = 0;
