...
/*   Updated: 2016/04/04 15:51:46 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/convert_boux.h"

void	ft_p_ushort(unsigned short n, int base, int *nb_written, \
	t_print param)
{
	char c;
	...
	{
		ft_p_ushort(n / base, base, nb_written, param);
		ft_p_ushort(n % base, base, nb_written, param);
	}
	else
	{
...
void	ft_calc_ushort(int nb_char[2], unsigned short n, \
	t_print param, int base)
{
	int	size;

	size = 0;
	ft_len_ushort(n, base, &size);
...
void	suite_ushort2(unsigned short n, t_print param, int *nb_written)
{
...
void	suite_ushort(unsigned short n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
	...
	suite_ushort2(n, param, nb_written);
...
int		ft_putnbr_ushort(unsigned short n, t_print param, int base)
{
	int	i;
	int	nb_char[2];
	int	nb_written;
	...
	i = 0;
	nb_written = 0;
	ft_calc_ushort(nb_char, n, param, base);
	suite_ushort(n, param, nb_char, &nb_written);
	...
	if (!(n == 0 && param.precision == 0 && param.precision_spec))
		ft_p_ushort(n, base, &nb_written, param);
	i = 0;
