...
/*   Updated: 2016/05/17 12:15:47 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/convert_d.h"

void	ft_p_int(int n, int *nb_written)
{
	if (n >= INT_MIN)
	{
		if (n >= 10)
		{
			ft_p_int(n / 10, nb_written);
			ft_p_int(n % 10, nb_written);
		}
		else if (n < 0)
		{
...
void	ft_calc_int(int nb_char[2], int n, t_print param)
{
	int	size;
...
int		suite_int_2(int n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
...
int		suite_int(int n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
	...
	i = suite_int_2(n, param, nb_char, nb_written);
...
int		ft_putnbr_int(int n, t_print param)
{
	int	i;
	int	nb_char[2];
	int	nb_written;
	...
	i = 0;
	nb_written = 0;
	ft_calc_int(nb_char, n, param);
	n = suite_int(n, param, nb_char, &nb_written);
	while (i < nb_char[0])
	{
		ft_putchar('0');
		i++;
