...
/*   Updated: 2016/04/04 14:36:04 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/convert_d.h"

void	ft_p_short(short n, int *nb_written)
{
	if (n >= SHRT_MIN)
	{
		if (n >= 10)
		{
			ft_p_short(n / 10, nb_written);
			ft_p_short(n % 10, nb_written);
		}
		else if (n < 0)
		{
...
void	ft_calc_short(int nb_char[2], short n, t_print param)
{
	int	size;
...
int		suite_short_2(short n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
...
short	suite_short(short n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
	...
	i = suite_short_2(n, param, nb_char, nb_written);
...
int		ft_putnbr_short(short n, t_print param)
{
	int	i;
	int	nb_char[2];
	int	nb_written;
	...
	i = 0;
	nb_written = 0;
	ft_calc_short(nb_char, n, param);
	n = suite_short(n, param, nb_char, &nb_written);
	while (i < nb_char[0])
	{
		ft_putchar('0');
		i++;
