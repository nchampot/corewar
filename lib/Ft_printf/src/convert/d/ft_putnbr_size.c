...
/*   Updated: 2016/04/04 14:46:47 by qmoreaux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../includes/convert_d.h"

void	ft_p_size(ssize_t n, int *nb_written)
{
	if (n >= LLONG_MIN)
	{
		if (n >= 10)
		{
			ft_p_size(n / 10, nb_written);
			ft_p_size(n % 10, nb_written);
		}
		else if (n < 0)
		{
...
void	ft_calc_size(int nb_char[2], ssize_t n, t_print param)
{
	int	size;
...
int		suite_size_2(ssize_t n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
...
ssize_t	suite_size(ssize_t n, t_print param, int nb_char[2], \
	int *nb_written)
{
	int	i;
	...
	i = suite_size_2(n, param, nb_char, nb_written);
...
int		ft_putnbr_size(ssize_t n, t_print param)
{
	int	i;
	int	nb_char[2];
	int	nb_written;
	...
	i = 0;
	nb_written = 0;
	ft_calc_size(nb_char, n, param);
	n = suite_size(n, param, nb_char, &nb_written);
	while (i < nb_char[0])
	{
		ft_putchar('0');
		i++;
